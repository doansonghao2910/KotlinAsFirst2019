...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val a = (grades.toList().groupBy({ it.second }, { it.first }))
    return (a.mapValues { (_, v) -> v.sorted() })
...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> {
    val c = a.distinct() + b.distinct()
    val list = mutableListOf<String>()
    val map = (c.groupBy { it }.mapValues { it.value.size })
    val list1 = map.values.toList()
    val list2 = map.keys.toList()
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    val x = word.toLowerCase().filter { set.add(it) }.toList()
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    for (i in 0 until list.size)
        for (a in i + 1 until list.size) {
            if (list[i] + list[a] == number) return Pair(i, a)
        }
    return Pair(-1, -1)
}
